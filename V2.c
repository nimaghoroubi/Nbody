//### version 2, cache miss due to large memory allocation
//########################################################
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
                if (i == j){
                    ...
                    dist[i][j] = eps + sqrt(((((*cluster_list[i]).pos_x - (*cluster_list[j]).pos_x)*(((*cluster_list[i]).pos_x - (*cluster_list[j]).pos_x)))
                            + (((*cluster_list[i]).pos_y - (*cluster_list[j]).pos_y)*(*cluster_list[i]).pos_y - (*cluster_list[j]).pos_y)));
                }else if(j<i){
                    dist[i][j] = dist[j][i];
                }
            }
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
                if(i == j){
                    ...
                    fx[i][j] = -((G*((*cluster_list[i]).mass)*((*cluster_list[j]).mass)/(dist[i][j]*dist[i][j]*dist[i][j]))*(((*cluster_list[i]).pos_x)-((*cluster_list[j]).pos_x)));
                    fx[i][i] = fx[i][i] + fx[i][j];
                    fy[i][j] = -((G*((*cluster_list[i]).mass)*((*cluster_list[j]).mass)/(dist[i][j]*dist[i][j]*dist[i][j]))*(((*cluster_list[i]).pos_y)-((*cluster_list[j]).pos_y)));
